...
const CurrentBox = (props) => {
  ...
  useEffect(() => {
    if (!locationKey) {
      dispatch(
        Actions.sendCitySelection(DEFUALT_LOCATION_KEY, DEFUALT_CITY, setCurrentWeather),
      );
    }
  ...
  return (
    ...
    >
        ...
        <CurrentText>{currentWeather.WeatherText}</CurrentText>
        <CurrentText>
          {currentWeather.Temperature.Imperial.Value +
            currentWeather.Temperature.Imperial.Unit}
        </CurrentText>
        <Logo source={currentWeather.iconPath} />
      </MainView>
  );
};
