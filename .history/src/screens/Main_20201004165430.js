        ...
        <MainView height={height} width={width}>
          ...
          {currentWeather && forecast && !error && (
            <WeatherView>
              ...
              <WeatherRow>
                <CurrentBox
                  height={isLandscape ? height * 0.49 : height * 0.24}
                ...
                {forecast.DailyForecasts.map((item, i) => {
                  return (
                    <ForecastBox
                      key={i}
                      item={item}
                      height={isLandscape ? height * 0.48 : height * 0.23}
...
const WeatherView = styled.View`
  width: 100%;
  display: flex;
  height: ${(props) => (props.isLandscape ? 40 : 85)}%;
`;
const WeatherRow = styled.View`
  width: 100%;
  height: ${(props) => (props.isLandscape ? 40 : 90)}%;
  display: flex;
  flex-direction: row;
  justify-content: center;
  align-items: center;
...
const SearchView = styled.View`
  width: 100%;
  flex-direction: row;
  height: ${(props) => (props.isLandscape ? 40 : 15)}%;
  justify-content: center;
  align-items: center;
