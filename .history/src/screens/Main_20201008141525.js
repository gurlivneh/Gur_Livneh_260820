        ...
        <MainView height={height} width={width}>
          ...
          {currentWeather && forecast && !error && (
            <WeatherColumn isLandscape={isLandscape}>
              <CurrentBox
                height={isLandscape ? height * 0.5 : height * 0.23}
                  ...
                  {forecast.DailyForecasts.map((item, i) => {
                    return (
                      <ForecastBox
                        key={i}
                        item={item}
                        height={isLandscape ? height * 0.55 : height * 0.25}
                        width={isLandscape ? width * 0.20 : width * 0.4}
                      />
                    );
                  })}
                </WeatherRow>
...
const WeatherRow = styled.View`
  width: 100%;
  height: ${(props) => props.height*0.75 + 60}px;
  display: flex;
  flex-direction: row;
  justify-content: center;
  align-items: center;
...
const WeatherColumn = styled.View`
  width: 100%;
  height: ${(props) => (props.isLandscape ? 60 : 95)}%;
  display: flex;
...
const UpperView = styled.View`
  width: 100%;
  flex-direction: row;
  height: ${(props) => (props.isLandscape ? 15 : 17)}%;
