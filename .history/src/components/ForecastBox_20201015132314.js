...
const ForecastBox = (props) => {
  ...
  const weatherNightIcon = props.item.Night.Icon < 10 ? "0"+props.item.Night.Icon : props.item.Night.Icon
  ...
  return (
    <>
      ...
      <ForecastText>{date[0]}</ForecastText>
        ...
        <Logo
          source={{ uri: `https://developer.accuweather.com/sites/default/files/${weatherDayIcon}-s.png` }}
        ...
        <Logo
          source={{ uri: `https://developer.accuweather.com/sites/default/files/${weatherNightIcon}-s.png` }}
        style={{width: 50, height: 50}} />
      </MainView>
    </>
  );
