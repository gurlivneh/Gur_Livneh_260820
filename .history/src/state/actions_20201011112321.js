...
export const getCurrentWeather = (locationKey, city, setCurrentWeather) => {
  return (dispatch, getState) => {
    dispatch(setCity(city));
    dispatch(setLocationKey(locationKey));
    dispatch(checkIsFavorite(locationKey)) 
    if (isMock) {
      setCurrentWeather(MockData.current[0])
    } else {
      Requests.getBylocationKey(locationKey).then((res) => {
        if (res.Code) {
           dispatch(setError(res.Message));
...
export const getForecast = (locationKey, setCurrentWeather) => {
