  ...
  useEffect(() => {
    try {
      AsyncStorage.getItem('favorites').then((res) => {
        ...
        if (!locationKey) {
          dispatch(Actions.setCity(DEFUALT_CITY));
          dispatch(Actions.setLocationKey(DEFUALT_LOCATION_KEY));
          dispatch(Actions.checkIsFavorite(DEFUALT_LOCATION_KEY));
        }

      });
    } catch (e) {
      alert('Failed to get the local storage data');
      if (!locationKey) {
        dispatch(Actions.setCity(DEFUALT_CITY));
        dispatch(Actions.setLocationKey(DEFUALT_LOCATION_KEY));
        dispatch(Actions.checkIsFavorite(DEFUALT_LOCATION_KEY));
